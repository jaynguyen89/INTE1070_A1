...
$response = '';
if (strlen($message) == 0) {
    $url = 'https://www.google.com/recaptcha/api/siteverify?response='.$token.'&secret=6LfHHbsZAAAAAMaNXaJz1_v-Wo3Xy7YO5Y7Zcb7S';
    $ch = curl_init();
    ...
    if (!$response) $message = 'Unable to validate your Recaptcha response. Please check network connection and retry.';
        ...
        if (!property_exists($jresponse, 'success') || !$jresponse->success ||
            (property_exists($jresponse, 'success') && $jresponse->success && $jresponse->score < 0.5)
        )
...
<div class="container">
    <div class="login-area">
        <h4><i class="fas fa-user-plus"></i> Create Account V3</h4>
        <h6>Please fill in your account details.</h6>

        <div class="login-row">
            <p id="validation" class="error"></p>
            <?php if (isset($_POST['submit_form']) && $message != '') { ?>
            ...
            <form action="./recaptcha_v3.php" method="post" id="registration-v3" autocomplete="off">
                <div class="col-sm-12" style="margin-bottom: 0.5rem;">
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input id="email" name="email" value="<?php echo $email; ?>" type="text" class="form-control" placeholder="Enter your email" oninput="validate(3)" autocomplete="none" />
                        </div>
                    </div>
                </div>
                <div class="col-sm-12" style="margin-bottom: 0.5rem;">
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input id="password" name="password" value="<?php echo $password; ?>" type="password" class="form-control" placeholder="Set your password" oninput="validate(3)" autocomplete="none" />
                        </div>
                ...
                <div class="col-sm-12" style="margin-bottom: 0.5rem;">
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <input id="confirm" name="confirm" value="<?php echo $confirm; ?>" type="password" class="form-control" placeholder="Confirm your password" oninput="validate(3)" autocomplete="none" />
                        </div>
                    </div>
                </div>
                <div class="col-sm-12" style="margin-bottom: 0.5rem;">
                ...
                <button id="submit-form" data-action="submit"
                        data-sitekey="6LfHHbsZAAAAABKdXd1rGZs_-zJhT-Zzol2SfPsc"
                        data-callback="confirmRegistration"
                        class="btn btn-primary g-recaptcha disabled">
                    <i class="fas fa-paper-plane"></i> Submit
                </button>
                <button class="btn btn-warning" onclick="clearForm(3)">Reset</button>
            </form>
        </div>
    </div>
</div>
